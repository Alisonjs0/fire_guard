...
import {Shield, AlertTriangle, CheckCircle, Clock, Wrench, TrendingUp, Calendar, MapPin, Bell, Filter, Download, BarChart3, PieChart, Activity, Users, Building, Eye, Plus, ArrowRight} from 'lucide-react'
import { useCRUD } from '../../hooks/useCRUD'
import { Extintor, Inspecao, Manutencao, Unidade, DashboardStats } from '../../types'
...
const Dashboard: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const extintoresConformes = extintores.filter(ext => 
      ext.status === 'conforme'
    ).length
    ...
    const manutencoesPendentes = manutencoes.filter(man => 
      man.status === 'agendada' || man.status === 'em_andamento'
    ).length
  ...
  const extintoresProximosVencimento = extintores.filter(ext => {
    ...
    return isAfter(dataValidade, new Date()) && isBefore(dataValidade, proximoMes)
  })
  ...
  const getStatusPercentage = (status: string) => {
    ...
    return Math.round((extintores.filter(e => e.status === status).length / extintores.length) * 100)
  }
  ...
  const alertasCriticos = [
    ...
    ...extintores.filter(e => e.status === 'vencido').map(ext => ({
      id: ext._id,
      tipo: 'vencido',
      titulo: `Extintor ${ext.numeroIdentificacao} vencido`,
      descricao: `Venceu em ${format(new Date(ext.dataValidade), 'dd/MM/yyyy')}`,
      urgencia: 'critica' as const
    })),
    ...manutencoes.filter(m => m.status === 'agendada').map(man => ({
      id: man._id,
      tipo: 'manutencao',
      titulo: `Manutenção agendada`,
      descricao: `${man.tipo} - ${format(new Date(man.dataAgendada), 'dd/MM/yyyy')}`,
  ...
  const estatisticasAvancadas = [
    {
      ...
      valor: `${Math.round((stats.extintoresConformes / stats.totalExtintores) * 100) || 0}%`,
      variacao: '+5%',
      tendencia: 'up',
      cor: 'green'
    },
                ...
                <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between">
                  <div>
                    ...
                    <p className="text-gray-600 mt-2">
                      Sistema de Gerenciamento de Extintores - {format(new Date(), 'dd/MM/yyyy', { locale: ptBR })}
                    </p>
                  </div>
                </div>
              </motion.div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
            ...
            <div className="bg-gradient-to-r from-red-50 to-orange-50 border border-red-200 rounded-xl p-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {alertasCriticos.slice(0, 3).map((alerta) => (
                  <div key={alerta.id} className="bg-white rounded-lg p-4 border border-red-100">
                    <div className="flex items-start space-x-3">
                      <div className={`w-3 h-3 rounded-full mt-1 ${
                        alerta.urgencia === 'critica' ? 'bg-red-500' :
                        alerta.urgencia === 'alta' ? 'bg-orange-500' : 'bg-yellow-500'
                      }`} />
                      <div className="flex-1">
                        <p className="font-medium text-gray-900 text-sm">{alerta.titulo}</p>
                        <p className="text-gray-600 text-xs mt-1">{alerta.descricao}</p>
                      </div>
                    </div>
                  </div>
                ))}
          ...
          {[
            ...
            >
              <div className="flex items-center justify-between">
                ...
                <div className={`w-12 h-12 rounded-lg flex items-center justify-center bg-${stat.cor}-100`}>
        ...
        >
          {estatisticasAvancadas.map((stat, index) => (
            <div key={stat.titulo} className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
              <div className="flex items-center justify-between mb-4">
                <h4 className="text-sm font-medium text-gray-600">{stat.titulo}</h4>
                <TrendingUp className={`w-4 h-4 ${
                  stat.tendencia === 'up' ? 'text-green-500' :
                  stat.tendencia === 'down' ? 'text-red-500' : 'text-gray-400'
                }`} />
              </div>
              <div className="flex items-end justify-between">
                <span className="text-2xl font-bold text-gray-900">{stat.valor}</span>
                <span className={`text-sm font-medium ${
                  stat.tendencia === 'up' ? 'text-green-600' :
                  stat.tendencia === 'down' ? 'text-red-600' : 'text-gray-500'
                }`}>
                  {stat.variacao}
                </span>
              </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 mb-8">
          ...
          >
            <div className="flex items-center justify-between mb-6">
              <h3 className="text-lg font-semibold text-gray-900">Status dos Extintores</h3>
              <PieChart className="w-5 h-5 text-gray-400" />
            </div>
            
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-6">
              {[
                { status: 'conforme', label: 'Conformes', color: 'green', count: extintores.filter(e => e.status === 'conforme').length },
                { status: 'nao_conforme', label: 'Não Conformes', color: 'red', count: extintores.filter(e => e.status === 'nao_conforme').length },
                { status: 'vencido', label: 'Vencidos', color: 'orange', count: extintores.filter(e => e.status === 'vencido').length },
                { status: 'manutencao', label: 'Em Manutenção', color: 'blue', count: extintores.filter(e => e.status === 'manutencao').length }
              ].map((item) => (
                <div key={item.status} className="text-center">
                  <div className={`w-16 h-16 mx-auto rounded-full bg-${item.color}-100 flex items-center justify-center mb-3`}>
                    <span className={`text-2xl font-bold text-${item.color}-600`}>{item.count}</span>
                  </div>
                  <p className="text-sm font-medium text-gray-900">{item.label}</p>
                  <p className="text-xs text-gray-500">{getStatusPercentage(item.status)}%</p>
                </div>
              ))}
            </div>
            ...
            {extintoresProximosVencimento.length > 0 ? (
              <div className="space-y-4">
                {extintoresProximosVencimento.slice(0, 5).map((extintor) => (
                  <div key={extintor._id} className="flex items-center p-3 bg-yellow-50 rounded-lg border border-yellow-200 hover:bg-yellow-100 transition-colors">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-xs text-yellow-700 font-medium">
                        Vence: {format(new Date(extintor.dataValidade), 'dd/MM/yyyy', { locale: ptBR })}
                      </p>
                    </div>
              ...
              <div className="text-center py-8">
                ...
                <p className="text-sm text-gray-500">Nenhum extintor próximo ao vencimento</p>
              </div>
            )}
          </motion.div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          ...
          >
            <div className="flex items-center justify-between mb-6">
              <h3 className="text-lg font-semibold text-gray-900">Últimas Inspeções</h3>
            ...
            {ultimasInspecoes.length > 0 ? (
              <div className="space-y-4">
                {ultimasInspecoes.map((inspecao) => (
                  <div key={inspecao._id} className="flex items-center p-4 bg-gray-50 rounded-lg hover:bg-gray-100 transition-colors">
                    <div className={`w-4 h-4 rounded-full mr-4 flex-shrink-0 ${
                      inspecao.resultado === 'conforme' ? 'bg-green-400' : 'bg-red-400'
                    }`} />
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-xs text-gray-500 mt-1">
                        {format(new Date(inspecao.dataInspecao), 'dd/MM/yyyy HH:mm', { locale: ptBR })}
                      </p>
              ...
              <div className="text-center py-8">
                ...
                <p className="text-sm text-gray-500">Nenhuma inspeção registrada</p>
          ...
          >
            ...
            <div className="space-y-4">
              {unidades.filter(u => u.ativo).slice(0, 5).map((unidade) => {
                const extintoresUnidade = extintores.filter(e => e.unidadeId === unidade._id)
                const conformes = extintoresUnidade.filter(e => e.status === 'conforme').length
                const total = extintoresUnidade.length
                const percentual = total > 0 ? Math.round((conformes / total) * 100) : 0
                
                return (
                  <div key={unidade._id} className="p-4 bg-gray-50 rounded-lg">
                    <div className="flex items-center justify-between mb-3">
                      <h4 className="font-medium text-gray-900 truncate">{unidade.nome}</h4>
                      <span className="text-sm text-gray-500">{total} extintores</span>
                    </div>
                    ...
                    <div className="flex items-center justify-between mb-2">
                      ...
                      <span className={`text-sm font-medium ${
                        ...
                        percentual >= 60 ? 'text-yellow-600' : 'text-red-600'
                      }`}>
                        {percentual}%
                      </span>
                    </div>
                    ...
                    <div className="w-full bg-gray-200 rounded-full h-2">
                      <div 
                        className={`h-2 rounded-full transition-all duration-1000 ${
                          ...
                          percentual >= 60 ? 'bg-yellow-500' : 'bg-red-500'
                        }`}
